...
object TcpEdgeIpRestrictionModule {
  private case class TcpEdgeIpRestrictionModuleReplace(
    module: Option[EndpointIpPolicyMutate]
  )

  private object TcpEdgeIpRestrictionModuleReplace {
    implicit val encodeTcpEdgeIpRestrictionModuleReplace: Encoder[TcpEdgeIpRestrictionModuleReplace] =
...
/** An API client for [[TcpEdgeIpRestrictionModule]].
  ...
  */
class TcpEdgeIpRestrictionModule private[ngrok] (apiClient: NgrokApiClient)(implicit ec: ExecutionContext) {
  import TcpEdgeIpRestrictionModule._
  ...
  def replace(
    ...
    apiClient.sendRequest[EndpointIpPolicy](
      ...
      s"/edges/tcp/$id/ip_restriction",
      List.empty,
      Option(
        TcpEdgeIpRestrictionModuleReplace(
          module
        ).asJson
      )
    )
